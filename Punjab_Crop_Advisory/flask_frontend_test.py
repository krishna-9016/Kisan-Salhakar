"""
Flask Frontend for Punjab Crop Advisory API Testing
==================================================

A simple Flask web application to test the Punjab Crop Advisory API.
This provides a user-friendly interface to input location, crop, and farm size
and get AI-powered crop predictions.
...
# Configuration
CROP_API_URL = "http://localhost:9090"
API_KEY = "punjab_crop_api_2024"

# Punjab districts with coordinates
PUNJAB_DISTRICTS = {
    ...
    "Gurdaspur": [32.0409, 75.4033],
    "Kapurthala": [31.3800, 75.3800],
    "Hoshiarpur": [31.5200, 75.9100],
    "Faridkot": [30.6700, 74.7600]
}

def call_crop_api(endpoint, method='GET', data=None):
    """Helper function to call the Crop Advisory API"""
    try:
        headers = {
        ...
        return {'success': False, 'error': 'Cannot connect to Crop Advisory API. Make sure it\'s running on port 9090.'}
...
@app.route('/')
def index():
    """Main page with crop advisory interface"""
    return render_template('crop_advisory.html', districts=PUNJAB_DISTRICTS)

@app.route('/api/health')
def check_api_health():
    """Check if the Crop Advisory API is running"""
    result = call_crop_api('/health')
    return jsonify(result)

@app.route('/api/crops')
def get_crops():
    """Get list of supported crops"""
    result = call_crop_api('/api/v1/crops')
    return jsonify(result)

@app.route('/api/predict', methods=['POST'])
def predict_crop():
    """Get crop prediction from the API"""
    try:
        # Get data from frontend
        data = request.json
        
        # Validate required fields
        required_fields = ['latitude', 'longitude', 'crop', 'acres']
        ...
        # Call the Crop Advisory API
        result = call_crop_api('/api/v1/predict', 'POST', api_data)
        return jsonify(result)
...
def create_template():
    """Create the HTML template for the frontend"""
...
<head>
    ...
    <title>🌾 Punjab Crop Advisory - Test Interface</title>
...
<body>
    <div class="main-container">
        <div class="header">
            <h1>🌾 Punjab Crop Advisory System</h1>
    ...
    <script>
        ...
        async function getPrediction() {
            ...
            try {
                ...
                btn.innerHTML = '🚀 Get AI Prediction';
                updatePredictButton();
            }
        }
        ...
        function displayResults(data) {
            ...
            if (data.crop_recommendations && data.crop_recommendations.length > 0) {
                html += '<h5 class="mt-4">🌱 Alternative Crop Recommendations</h5>';
            ...
            if (data.farming_tips && data.farming_tips.length > 0) {
                html += '<h5 class="mt-4">💡 Farming Tips</h5>';
    ...
    template_path = os.path.join(templates_dir, 'crop_advisory.html')
...
if __name__ == '__main__':
    print("🌾 Punjab Crop Advisory - Flask Frontend")
    print("=" * 50)
    ...
    print("🔗 API: http://localhost:9090")
    print("\n⚠️  Make sure your Crop Advisory API is running:")
    print("   cd Punjab_Crop_Advisory")
    print("   python crop_prediction_api.py")
    print("=" * 50)
